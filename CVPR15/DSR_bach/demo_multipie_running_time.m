...
detection_result = load('multipie_bboxes.mat', 'allboxes');
mpie_bboxes = detection_result.allboxes;
clear detection_result

canonical_size = [240 320];
scale_ratio = 0.25;
...
% for sub = 1:1
    ...
    path = fullfile('/Users/yeqing/Documents/Data/multipie/session01', subject, '01', '05_1');
    ...
    for i=1:LengthFiles
        ...
        im = mean(im,3);
        im = imresize(im,canonical_size*scale_ratio);
        %     im = 255*im/max(im(:));
        I0(:,:,i) = im;
    ...
    if false,
        ...
        bboxes = mpie_bboxes{sub} * scale_ratio;
    end
    ...
    time_dsr_100(sub,:) =  mean(time_dsr(:));
    time_rasl_100(sub,:) =  mean(time_rasl(:));
    time_tgra_100(sub,:) =  mean(time_gra(:));
