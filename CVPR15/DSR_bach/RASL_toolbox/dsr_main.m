function [Din, Do, A, E, xi, numIterOuter, numIterInner ] = dsr_main(fileNames, transformations, numImages, raslpara, destDir)
...
for fileIndex = 1 : numImages
    ...
    currentImage = single(imread(fileNames{fileIndex}));
    ...
    Iori(:,:,fileIndex) = currentImage;
    currentImage = grad(currentImage); 
...
%D = [] ;
D= zeros(imgSize(1)*imgSize(2), numImages,'single');     Din = D;
for fileIndex = 1 : numImages
    % transformed image
    ...
    I   = vec(imtransform(Iori(:,:,fileIndex), Tfm,'bicubic','XData',[1 imgSize(2)],'YData',[1 imgSize(1)],'Size',imgSize));
    y   = I; 
    y = y / norm(y) ; % normalize
    Din(:,fileIndex) = y ;
...
for scaleIndex = raslpara.numScales:-1:1 % multiscale
    ...
    while ~converged
        ...
        %D = [] ;
        D= zeros(imgSize(1)*imgSize(2), numImages,'single');
        J = cell(1,numImages);
        ...
        if APGorALM_flag == 1
            ...
            [A, E, delta_xi, numIterInnerEach] = dsr_inner_ialm2(D, Q, lambda, raslpara.inner_tol, raslpara.inner_maxIter);
%               [A, E, delta_xi, numIterInnerEach] = dsr_inner(D, Q, raslpara.alpha, raslpara.beta, raslpara.inner_tol, raslpara.inner_maxIter,imgSize);
%             [A, E, delta_xi, numIterInnerEach] = dsr_inner_fast(D, Q, raslpara.alpha, raslpara.beta, raslpara.inner_tol, raslpara.inner_maxIter,imgSize);
        end
