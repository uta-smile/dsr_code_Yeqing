function [A_dual E_dual dt_dual iter Y] = dsr_inner_ialm2(D, J, lambda, tol, maxIter)
...
while ~converged       
    ...
    A_dual = wthresh(FT,'s',1/mu);
    A_dual(:,1) = FT(:,1);
    A_dual = ifft(A_dual,[],2);
...
%     end    
    ...
    if stoppingCriterion <= tol
        ...
        disp(['Iteration ' num2str(iter) '  rank(A) ' ...
            ' ||E||_0 '  '  Stopping Criterion ' ...
            num2str(stoppingCriterion)]) ;
        converged = true ;
    end
